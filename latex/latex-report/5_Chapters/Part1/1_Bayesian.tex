...
If the prior density is of the same form, with its own parametrization of $a$ and $b$, then the posterior will also be of this form. Such a prior density can be parameterized as: 
...
The beta distribution is defined on the interval $[0, 1]$. .. shows the beta distribution with different shape parameters. 
...
However, we do not know $\theta$. In the Bayesian paradigm, when we do not know the value of a parameter, we put a prior on it. As a prior, we will use a **beta distribution**, which looks as follows: 


The factor $\binom{n}{y}$ does not depend on the unknown parameter $\theta$, and can be treated as a constant when calculating the posterior distribution of $\theta$ when we regard $n$ and $y$ as part of the experimental design that is considered fixed. 
