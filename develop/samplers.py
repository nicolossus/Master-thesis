"""
https://github.com/pymc-devs/pymc4/blob/master/pymc4/mcmc/samplers.py
"""
...
from pymc4.mcmc.utils import (KERNEL_KWARGS_SET, initialize_sampling_state,
                              initialize_state,
...
__all__ = ["HMC", "NUTS", "RandomWalkM",
           "CompoundStep", "NUTSSimple", "HMCSimple"]
reg_samplers = {}
...
class _BaseSampler(metaclass=abc.ABCMeta):
    ...
    def _sample(
        ...
        *,
        ...
        if include_log_likelihood:
            log_likelihood_dict = calculate_log_likelihood(
                self.model, posterior, state_)
    ...
    def _assign_arguments(self, kwargs):
        ...
        adaptation_keys = (
            set(list(inspect.signature(
                self._adaptation.__init__).parameters.keys())[1:])
            if self._adaptation
            else set()
        )
        kernel_keys = set(list(inspect.signature(
            self._kernel.__init__).parameters.keys())[1:])
        chain_keys = set(list(inspect.signature(
            mcmc.sample_chain).parameters.keys()))
        ...
        self.adaptation_kwargs = {k: kwargs[k]
                                  for k in (adaptation_keys & kwargs_keys)}
        self.kernel_kwargs = {k: kwargs[k]
                              for k in (kernel_keys & kwargs_keys)}
...
class CompoundStep(_BaseSampler):
    ...
    def _merge_samplers(self, make_kernel_fn, part_kernel_kwargs):
        ...
        self.parent_inds = sorted(
            range(len(parents)), key=lambda k: parents[k])
    ...
    def _assign_default_methods(
        ...
        *,
        ...
        kernels, set_lengths = self._merge_samplers(
            make_kernel_fn, part_kernel_kwargs)
...
def build_logp_and_deterministic_functions(
    ...
    if collect_reduced_log_prob:
        ...
        def logpfn(*values, **kwargs):
            if kwargs and values:
                raise TypeError(
                    "Either list state should be passed or a dict one")
            elif values:
                kwargs = dict(zip(unobserved_keys, values))
            st = flow.SamplingState.from_values(
                kwargs, observed_values=observed)
    ...
    def deterministics_callback(*values, **kwargs):
        if kwargs and values:
            ...
            kwargs = dict(zip(unobserved_keys, values))
        st = flow.SamplingState.from_values(
            kwargs, observed_values=observed_var)
        _, st = flow.evaluate_model_transformed(model, state=st)
...
def calculate_log_likelihood(
    ...
    for observed_rv in sampling_state.observed_values:
        extract_log_likelihood = partial(
            extract_log_likelihood, observed_rv=observed_rv)
